...
payfast_transaction_action_mapping = {'Completed': TransactionAction.complete,
                                     'Denied': TransactionAction.reject,
                                     'Pending': TransactionAction.pending}


class RHPayfastIPN(RH):
    """Process the notification sent by the Payfast"""
    ...
    def _process(self):
        ...
        if result != 'VERIFIED':
            current_plugin.logger.warning("Payfast IPN string %s did not validate (%s)", verify_params, result)
            return
        ...
        if payment_status == 'Refunded' or float(request.form.get('mc_gross')) <= 0:
            ...
            return
        if payment_status not in payfast_transaction_action_mapping:
        ...
        register_transaction(registration=self.registration,
                             ...
                             action=payfast_transaction_action_mapping[payment_status],
                             provider='payfast',
                             data=request.form)
    ...
    def _is_transaction_duplicated(self):
        ...
        if not transaction or transaction.provider != 'payfast':
            return False
...
class RHPayfastSuccess(RHPayfastIPN):
...
class RHPayfastCancel(RHPayfastIPN):
