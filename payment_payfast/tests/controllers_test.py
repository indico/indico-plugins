...
from indico_payment_payfast.controllers import RHPayfastIPN
from indico_payment_payfast.plugin import PayfastPaymentPlugin
...
def test_ipn_verify_business(formdata, expected, dummy_event):
    ...
    PayfastPaymentPlugin.event_settings.set(dummy_event, 'business', 'test')
    request.form = formdata
    with PayfastPaymentPlugin.instance.plugin_context():
...
def test_ipn_verify_amount(mocker, amount, currency, expected):
    nai = mocker.patch('indico_payment_payfast.controllers.notify_amount_inconsistency')
    rh = RHPayfastIPN()
...
def test_ipn_is_transaction_duplicated(txn_id, payment_status, expected):
    ...
    transaction = PaymentTransaction(provider='payfast', data={'payment_status': payment_status, 'txn_id': txn_id})
...
def test_ipn_process(mocker, fail):
    rt = mocker.patch('indico_payment_payfast.controllers.register_transaction')
    post = mocker.patch('indico_payment_payfast.controllers.requests.post')
    mocker.patch('indico_payment_payfast.controllers.notify_amount_inconsistency')
